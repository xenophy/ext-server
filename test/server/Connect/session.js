...
describe('Ext.server.Connect.session', function() {
    ...
    var app = Ext.create('Ext.server.Connect')
              ...
              .use(Ext.server.Connect.session({ cookie: { maxAge: min }}))
              .use(respond);

    it('should export constructors', function() {
        Ext.server.Connect.session.Session.should.be.a('function');
        Ext.server.Connect.session.Store.should.be.a('function');
        Ext.server.Connect.session.MemoryStore.should.be.a('function');
    });
    ...
    describe('proxy option', function() {
        describe('when enabled', function() {
            it('should trust X-Forwarded-Proto', function(done) {
                var app = Ext.create('Ext.server.Connect')
                          .use(Ext.server.Connect.cookieParser('keyboard cat'))
                          .use(Ext.server.Connect.session({ proxy: true, cookie: { secure: true, maxAge: 5 }}))
                          .use(respond);
        ...
        describe('when disabled', function() {
            it('should not trust X-Forwarded-Proto', function(done) {
                ...
                var app = Ext.create('Ext.server.Connect')
                          .use(Ext.server.Connect.cookieParser('keyboard cat'))
                          .use(Ext.server.Connect.session({ cookie: { secure: true, maxAge: min }}))
                          .use(respond);
    ...
    describe('key option', function() {
        ...
        it('should allow overriding', function(done) {
            var app = Ext.create('Ext.server.Connect')
            .use(Ext.server.Connect.cookieParser('keyboard cat'))
            .use(Ext.server.Connect.session({ key: 'sid', cookie: { maxAge: min }}))
            .use(respond);
    ...
    describe('req.session', function() {
        ...
        describe('.destroy()', function() {
            it('should destroy the previous session', function(done) {
                var app = Ext.create('Ext.server.Connect')
                          .use(Ext.server.Connect.cookieParser('keyboard cat'))
                          .use(Ext.server.Connect.session())
                          .use(function(req, res, next) {
        ...
        describe('.cookie', function() {
            ...
            describe('.*', function() {
                ...
                it('should serialize as parameters', function(done) {
                    var app = Ext.create('Ext.server.Connect')
                              .use(Ext.server.Connect.cookieParser('keyboard cat'))
                              .use(Ext.server.Connect.session({ proxy: true, cookie: { maxAge: min }}))
                              .use(function(req, res, next) {
                ...
                it('should default to a browser-session length cookie', function(done) {
                    var app = Ext.create('Ext.server.Connect')
                              .use(Ext.server.Connect.cookieParser('keyboard cat'))
                              .use(Ext.server.Connect.session({ cookie: { path: '/admin' }}))
                              .use(function(req, res, next) {
                                  res.end();
                              });
                ...
                it('should override defaults', function(done) {
                    var app = Ext.create('Ext.server.Connect')
                              .use(Ext.server.Connect.cookieParser('keyboard cat'))
                              .use(Ext.server.Connect.session({ cookie: { path: '/admin', httpOnly: false, secure: true, maxAge: 5000 }}))
                              .use(function(req, res, next) {
            ...
            describe('.maxAge', function() {
                it('should set relative in milliseconds', function(done) {
                    ...
                    .get('/')
                    .end(function(res) {
                        var a = new Date(expires(res)), b = new Date;

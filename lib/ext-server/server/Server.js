...
Ext.define('Ext.server.Server', {
    ...
    constructor: function(config) {
        ...
        config.ws = (config.ws !== false) ? true : false;
        ...
        init = function() {
            ...
            app.use(function(req, res, next) {
                ...
                ip  = req.headers['x-forwarded-for'] || req.client.remoteAddress;
                f   = Ext.util.GeoIP.getCountry;

                req.geoip = { code: f(ip, 'code'), code3: f(ip, 'code3'), name: f(ip) };
            ...
            next = function() {
                ...
                if(config.ssl) {
                    ...
                    if(config.wss) {
                        ...
                        Ext.ws = Ext.create(
                            'Ext.server.WebSocket',
                            Ext.$dependencies['socket.io'].listen(httpsServer, Ext.isSimpleObject(config.wss) ? config.wss : {}),
                            me.config,
                            true,
                            next
                        );
            ...
            if(config.ws) {
                ...
                Ext.ws = Ext.create(
                    'Ext.server.WebSocket',
                    Ext.$dependencies['socket.io'].listen(httpServer, Ext.isSimpleObject(config.ws) ? config.ws : {}),
                    me.config,
                    false,
                    next
                );
